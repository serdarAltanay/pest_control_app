...
const fmtDateTime = (d) =>
  d.toLocaleString("tr-TR", { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" });
...
async function updateStatus(id, status) {
  try {
    await api.put(`/schedule/events/${id}`, { status });
    return true;
  } catch (e) {
...
export default function VisitDetail() {
  ...
  const plannedByDisplay = useMemo(() => {
    if (!event) return "-";
    return event.plannedByName || "-";
  }, [event]);

  const durationMin = useMemo(() => {
    if (!event) return 0;
    return Math.round((event.end - event.start) / (60 * 1000));
  }, [event]);
  ...
  if (!event) {
    return (
      <Layout title="Ziyaret Detayı">
        <div className="vd-page">
          <div className="card">Ziyaret bulunamadı.</div>
        </div>
      </Layout>
    );
  }
      ...
      <div className="vd-page">
        ...
        <div className="vd-head card">
          ...
          <div className="actions">
            {store?.id && (
              <Link className="btn ghost" to={`/stores/${store.id}`}>Mağaza Detayı</Link>
            )}
            {store?.id && (
        ...
        <div className="vd-grid">
          ...
          <div className="card info">
            ...
            <div className="kv">
              <div className="k">Tarih</div>
              <div className="v">{fmtDate(event.start)}</div>
            </div>

            <div className="kv">
              <div className="k">Saat</div>
              <div className="v">
                {fmtTime(event.start)} – {fmtTime(event.end)} ({durationMin} dk)
              </div>
            </div>
            ...
            <div className="kv">
              ...
              <div className="v">
                <span
                  className="chip"
                  style={{ background: event.color, color: "#0b1220" }}
            ...
            {/* Planlayan */}
            <div className="kv">
              <div className="k">Planlayan</div>
              <div className="v">{plannedByDisplay}</div>
            </div>

            <div className="kv">
              <div className="k">Planlama Zamanı</div>
              <div className="v">{event.plannedAt ? fmtDateTime(event.plannedAt) : "-"}</div>
            </div>
            ...
            {event.storeName && (
              <div className="kv">
                ...
                <div className="v">{event.storeName}</div>
              </div>
            )}
            ...
            {event.notes && (
              <div className="kv">
                <div className="k">Notlar</div>
                <div className="v">{event.notes}</div>
              </div>
            )}
          ...
          <div className="card info">
            ...
            {store ? (
              <>
                <div className="kv">
                  <div className="k">Adı</div>
                  <div className="v">{store.code ? `${store.code} – ` : ""}{store.name}</div>
                </div>
                {store.city && (
                  <div className="kv">
                    <div className="k">Şehir</div>
                    <div className="v">{store.city}</div>
                  </div>
                )}
                {store.address && (
                  <div className="kv">
                    <div className="k">Adres</div>
                    <div className="v">{store.address}</div>
                  </div>
                )}
                {(store.manager || store.phone) && (
                  <div className="kv">
                    <div className="k">İrtibat</div>
                    <div className="v">
                      {store.manager || "-"} {store.phone ? ` · ${store.phone}` : ""}
                    </div>
                  </div>
                )}
                <div className="kv">
                  <div className="k">Durum</div>
                  <div className="v">{store.isActive ? "Aktif" : "Pasif"}</div>
                </div>
              </>
            ) : (
        ...
        <div className="card map-card">
          <div className="sec-title row">
            <span>Konum</span>
            {directionsUrl && (
              <a className="btn ghost" href={directionsUrl} target="_blank" rel="noopener noreferrer">
                Yol Tarifi
              </a>
            )}
          </div>
        ...
        <div className="vd-foot">
          ...
          {store?.id && <Link className="btn ghost" to={`/stores/${store.id}`}>Mağaza Detayı</Link>}
          {store?.id && <Link className="btn" to={`/admin/stores/${store.id}/ek1`}>EK-1 Rapor Doldur</Link>}
        </div>
      </div>
    </Layout>
  );
